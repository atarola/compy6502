...
const ANSI_TO_TEXTVGA: [u8; 16] = [
    0x00, 0x04, 0x02, 0x06, 0x01, 0x05, 0x03, 0x07, 0x08, 0x0C, 0x0A, 0x0E, 0x09, 0x0D, 0x0B,
    0x0F,
];
...
impl Text {
    ...
    async fn push_state(&self, driver: &mut DisplayHandle) {
        driver.begin_bulk(EVE_RAM_G).await;
        for (r, row) in self.grid.iter().enumerate() {
            for (c, cell) in row.iter().enumerate() {
                let (ch, attr) = if self.cursor_visible
                    && r == self.cursor_y as usize
                    && c == self.cursor_x as usize
                {
                    (0xDB, self.attr)
                } else {
                    (cell.ch, cell.attr)
...
impl AnsiParser {
    ...
    fn feed(&mut self, c: u8) -> Option<AnsiCmd> {
        match self.state {
            AnsiState::Normal => self.on_normal(c),
    ...
    fn on_normal(&mut self, c: u8) -> Option<AnsiCmd> {
        match c {
            ...
            0x0D | 0x0A => Some(AnsiCmd::Newline),
            0x08 | 0x7F => Some(AnsiCmd::Backspace),
            _ => Some(AnsiCmd::PutChar(c)),
        }
    }

    fn on_esc(&mut self, c: u8) -> Option<AnsiCmd> {
        if c == 0x5B {
            self.state = AnsiState::Sequence(Vec::new());
            return None;
        }
    ...
    fn parse_cursor_up(params: &[u8; 8]) -> Option<AnsiCmd> {
        Some(AnsiCmd::CursorUp(if params[0] == 0 { 1 } else { params[0] }))
    }

    fn parse_cursor_down(params: &[u8; 8]) -> Option<AnsiCmd> {
        Some(AnsiCmd::CursorDown(if params[0] == 0 { 1 } else { params[0] }))
    }

    fn parse_cursor_right(params: &[u8; 8]) -> Option<AnsiCmd> {
        Some(AnsiCmd::CursorRight(if params[0] == 0 { 1 } else { params[0] }))
    }

    fn parse_cursor_left(params: &[u8; 8]) -> Option<AnsiCmd> {
        Some(AnsiCmd::CursorLeft(if params[0] == 0 { 1 } else { params[0] }))
    }
...
impl DisplayMode for Text {
    ...
    fn consume(&mut self, _buf: &mut CommandBuffer, byte: u8) {
        if let Some(cmd) = self.parser.feed(byte) {
            self.apply(cmd);
        }
    }
