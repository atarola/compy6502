...
class TestKernelStringsStrCopy(BaseTest):
    def test_strings_str_copy(self):
        cpu = self.get_rom_cpu()

        cpu.set_k_ptr(DEST)
        cpu.set_k_ptr2(SOURCE)
        cpu.poke(SOURCE, bytes([0x03, ord("A"), ord("B"), ord("C")]))
        cpu.poke(DEST, bytes([0x02, ord("X"), ord("Y")]))

        self.run_sub(cpu, "STR_COPY")
    ...
    def test_strings_str_copy_empty(self):
        cpu = self.get_rom_cpu()

        cpu.set_k_ptr(DEST)
        cpu.set_k_ptr2(SOURCE)
        cpu.poke(SOURCE, bytes([0x00]))
        cpu.poke(DEST, bytes([0x02, ord("X"), ord("Y")]))
